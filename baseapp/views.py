...
def cabinet_view(request):
    ...
    if form.is_valid(): form.save(request.user)
    if request.method == 'GET': form = SettingsForm(instance=request.user)
    if form2.is_valid():
        user = form2.save(request.user.email)
        if user is not None: form2 = UserPasswordChangeForm()
